...
namespace SharpNeat.Domains.PreyCapture
{
    ...
    /// </summary>
    partial class PreyCaptureView : AbstractView
    {
        // View painting consts & objects.
        private const int GridTop = 2;
        private const int GridLeft = 2;
        private const PixelFormat ViewportPixelFormat = PixelFormat.Format16bppRgb565;
        private static readonly Pen __penGrey = new Pen(Color.LightGray, 1F);
        private readonly Brush _brushBackground = new SolidBrush(Color.Lavender);
        private readonly Brush _brushBackgroundSensor = new SolidBrush(Color.LightBlue);
        private readonly Brush _brushBackgroundSensorCaptured = new SolidBrush(Color.Orange);
        private readonly Brush _brushAgent = new SolidBrush(Color.Red);
        private readonly Brush _brushPrey = new SolidBrush(Color.Green);

        private IGenomeDecoder<NeatGenome, IBlackBox> _genomeDecoder;
        private PreyCaptureWorld _world;
        ...
        /// </summary>
        private AutoResetEvent _simStartEvent = new AutoResetEvent(false);
        ...
        public PreyCaptureView(IGenomeDecoder<NeatGenome, IBlackBox> genomeDecoder, PreyCaptureWorld world)
        {
            try
            {
                ...
                _simThread = new Thread(SimulationThread);
        ...
        private void RunTrial()
        {
            ...
            for (; t < _world.PreyInitMoves; t++)
            {
                ...
                if (!this.IsDisposed)
                    Invoke(new MethodInvoker(PaintView));
        ...
        private void PaintView()
        {
            ...
            var visualFieldPixelSize = (height - GridTop)/_world.GridSize;
            width = height = visualFieldPixelSize*_world.GridSize;

            // Paint pixel outline grid.
            // Vertical lines.
            var xg = GridLeft;
            for (var i = 0; i <= _world.GridSize; i++, xg += visualFieldPixelSize)
            {
            ...
            // Horizontal lines.
            var yg = GridTop;
            for (var i = 0; i <= _world.GridSize; i++, yg += visualFieldPixelSize)
            {
                g.DrawLine(__penGrey, GridLeft, yg, GridLeft + width, yg);
            }

            // Paint grid squares. Background color.
            var sensorBrush = _world.IsPreyCaptured() ? _brushBackgroundSensorCaptured : _brushBackgroundSensor;

            yg = GridTop;
            for (var y = 0; y < _world.GridSize; y++, yg += visualFieldPixelSize)
            {
                xg = GridLeft;
                for (var x = 0; x < _world.GridSize; x++, xg += visualFieldPixelSize)
                {
            ...
            var a = _world.AgentPosition;
            var p = _world.PreyPosition;
            g.FillRectangle(_brushAgent, GridLeft + (a._x*visualFieldPixelSize) + 1,
                GridTop + (a._y*visualFieldPixelSize) + 1, visualFieldPixelSize - 2, visualFieldPixelSize - 2);
            g.FillRectangle(_brushPrey, GridLeft + (p._x*visualFieldPixelSize) + 1,
                GridTop + (p._y*visualFieldPixelSize) + 1, visualFieldPixelSize - 2, visualFieldPixelSize - 2);
